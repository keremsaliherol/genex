...
// 5) Fontlar (Fontsource, OFL-1.1), yalnız latin ve latin-ext alt kümeleri: Türkçe ğ, ş, ı, İ ve ₺ latin-ext'te.
//    Geist (arayüz) ve Geist Mono (veri) ağırlık eksenli; Mona Sans (başlık ve büyük rakamlar) ağırlık + genişlik
//    eksenli: başlıklarda geniş, rakamlarda dar kullanılır. @font-face blokları paketin CSS'inden alınır; aile adı tema
//    token'larındaki adlara (--font-sans, --font-mono, --font-display), adresler wwwroot/fonts'a çevrilir.
const fontHedef = join(wwwroot, 'fonts');
mkdirSync(fontHedef, { recursive: true });
const fontYuzleri = [];
const fontlar = [['geist', 'Geist', 'index.css', 'wght'], ['geist-mono', 'Geist Mono', 'index.css', 'wght'], ['mona-sans', 'Mona Sans', 'wdth.css', 'wdth']];
for (const [paket, aile, cssDosyasi, eksen] of fontlar) {
  ...
  const css = readFileSync(join(klasor, cssDosyasi), 'utf8');
  const altKumeDeseni = new RegExp(`-latin(-ext)?-${eksen}-normal$`);
  for (const [, altKume, blok] of css.matchAll(/\/\* ([\w-]+) \*\/\s*(@font-face \{[^}]+\})/g)) {
    if (!altKumeDeseni.test(altKume)) continue;
  ...
  copyFileSync(join(klasor, 'LICENSE'), join(fontHedef, `OFL-${aile.replace(/ /g, '')}.txt`));
}
if (fontYuzleri.length !== 6) throw new Error(`6 @font-face bekleniyordu, ${fontYuzleri.length} bulundu: Fontsource CSS biçimi değişmiş olabilir.`);
writeFileSync(join(wwwroot, 'css', 'fontlar.css'), `/* Geist, Geist Mono ve Mona Sans, SIL Open Font License 1.1 (fonts/OFL-*.txt).
   Bu dosya üretilir, elle değiştirmeyin: node tools/on-yuz-varliklari.mjs
   Kaynak: @fontsource-variable/geist, geist-mono ve mona-sans (prototype/node_modules). */\n\n${fontYuzleri.join('\n\n')}\n`, 'utf8');

...
BEGIN
    ...
    -- Kimlik tabloları (05_identity.sql). Yönetici hesabı uygulamanın bir sonraki açılışında yeniden oluşturulur.
    sil('DROP TABLE ASPNET_USER_TOKENS CASCADE CONSTRAINTS PURGE');
    sil('DROP TABLE ASPNET_USER_LOGINS CASCADE CONSTRAINTS PURGE');
    sil('DROP TABLE ASPNET_USER_CLAIMS CASCADE CONSTRAINTS PURGE');
    sil('DROP TABLE ASPNET_USERS CASCADE CONSTRAINTS PURGE');
END;
/
